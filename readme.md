...
### 一款全新的Cloudflare R2云盘，支持带密码分享、查看下载量、管理用户等功能

本项目为原版 [EdgeStash](https://github.com/hhy-2021/EdgeStash) 的增强修改版，倾向于自用，有些功能用不到的可能不会考虑增加。

<span style="color:red;">AI 生成/修改项目，不喜欢可以直接关闭本页。</span>

本项目同时也更换了界面UI（做了些修改），感谢 https://github.com/Rex2516S
...
- **👑多用户系统**：
  ...
  - **访客用户**：使用密码下载被分享文件，无密码保护的分享文件可直接下载，如有需要可以不登陆来浏览指定文件夹和下载文件，其余权限均禁止。
...
可以用访客的身份直接浏览文件夹和下载文件，不用登录，但首先必须要到用户管理中增加一个特定的用户：

```
电子邮箱：guest@guest.com

密码：留空则可以实现不用登录访问

访客首页文件夹：默认为根目录，限制访客可以浏览和下载的文件夹

```

更好的 UI ，支持列表（默认）和网格视图，响应式设计增强触摸设备上的体验
...
| **文件管理** |                            |                            |
| `GET`  | `/api/files?path=<path>`                 | 获取指定路径下的文件和文件夹 |
| `POST` | `/api/files?path=<path>`                 | 上传文件到指定路径         |
| `PUT`  | `/api/files?path=<path>`                 | 重命名文件或文件夹         |
| `DELETE`| `/api/files?path=<path>`                 | 删除文件或文件夹           |
| `POST` | `/api/folders`                 | 创建新文件夹               |
| `GET`  | `/api/download?path=<path>`              | 下载文件                   |
| `GET`  | `/api/preview?path=<path>`               | 获取文件内容用于在线预览   |
| **分享管理** |                            |                            |
| `POST` | `/api/share`                   | 为文件创建分享链接         |
| `GET`  | `/api/share/{shareId}`               | 获取分享链接信息           |
| `POST` | `/api/share/{shareId}/download`      | 下载分享的文件             |
| **管理后台** |                            |                            |
| `GET`  | `/api/admin/stats`             | 获取统计数据               |
| `GET`  | `/api/admin/shares`            | 列出所有分享链接           |
| `DELETE`| `/api/admin/shares/{shareId}`        | 删除指定的分享链接         |
| `GET`  | `/api/admin/users`             | 列出所有用户列表           |
| `POST` | `/api/admin/users`             | 创建新用户                 |
| `GET` | `/api/admin/users/{email}` | 获取单个用户详情 |
| `PUT` | `/api/admin/users/{email}` | 更新用户信息 |
| `DELETE`| `/api/admin/users/{email}`      | 删除指定用户               |

所有路径中的 `{email}` 和 `{shareId}` 需要进行 URL 编码，且 `GET /api/files` 和 `POST /api/files` 等接口通过 `?path=` 参数传递路径信息，路径需进行 `encodeURIComponent` 编码。
...
[![Stargazers over time](https://starchart.cc/Little-Data/EdgeStash-mod.svg?variant=adaptive)](https://starchart.cc/Little-Data/EdgeStash-mod)
